...
int main(int argc, char **argv)
{
	...
	maxidx=0;
	if (!(fp =fopen(ROIfile, "r"))) errr("roieval3", ROIfile);
	rois.List = (ROI_Info *)malloc((size_t)(NROI*sizeof(ROI_Info)));
	if (!rois.List) errm("roieval3");
	...
	while (fgets(line, 512, fp))
	{
		...
		if (maxidx<rip->MaskVal) maxidx=rip->MaskVal;
		rip++;
		i++;	
	}
	...
	/* Get raw roi means */
	roimean=vector(1,maxidx);
	roinv=vector(1,maxidx);
	getroismean2_4dfp(frame, RSFMask, PETMask, maxidx, roimean, roinv);
	
	
	/* Write output file */
	sprintf(f1, "%s_ROI2_f%d", PETID, f);
	if (!(fp =fopen(f1, "w"))) errw("roieval3", f1);
	fprintf(fp, "%-35s %10s %16s\n", "Structure_Name", "NVoxels", "Mean Intensity");

	for (i=0; i<NROI; i++){
		fprintf(fp, "%-35s %10d %16.4f\n", rois.List[i].Name, (int)roinv[rois.List[i].MaskVal+1], roimean[rois.List[i].MaskVal+1]);
	}
	fclose(fp);
}
