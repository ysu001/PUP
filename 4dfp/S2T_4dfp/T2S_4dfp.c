/*$Header: /data/petsun4/data1/src_solaris/S2T_4dfp/RCS/T2S_4dfp.c,v 1.4 2007/08/31 05:18:24 avi Exp $*/
/*$Log: T2S_4dfp.c,v $
 ...
 *
 * Revision 1.1  2004/03/11  07:04:12  avi
 * Initial revision
 **/
...
/* externals */
/*************/
extern void	t2sf_ (float *imgm, int *nx, int *nz, int *ny, float *imgn);	/* reorder.f */
...
static char rcsid[]= "$Id: T2S_4dfp.c,v 1.4 2007/08/31 05:18:24 avi Exp $";
...
/************************/
	...
	if (k < 1) {
		...
		printf (" e.g.,\t%s vm6c_mpr vm6c_mprS\n", program);
		printf ("\toption\n");
		printf ("\t-@<b|l>\toutput big or little endian (default input endian)\n");
		printf ("N.B.:	default output root = <imgroot>\"S\"\n");
		exit (1);
	}
	if (!strlen (outroot)) sprintf (outroot, "%sS", imgroot); 
...
/************/
	...
	for (k = 0; k < imgdim[3]; k++) {
		...
		t2sf_ (img1, imgdim + 0, imgdim + 1, imgdim + 2, img2);
		flipy (img2, imgdim + 1, imgdim + 2, imgdim + 0);
		flipz (img2, imgdim + 1, imgdim + 2, imgdim + 0);
...
/* create ifh file */
/*******************/
	imgdimo[0] = imgdim[1]; voxdimo[0] = voxdim[1];
	imgdimo[1] = imgdim[2]; voxdimo[1] = voxdim[2];
	imgdimo[2] = imgdim[0]; voxdimo[2] = voxdim[0];
	imgdimo[3] = imgdim[3];
	writeifhe (program, outfile, imgdimo, voxdimo, 4, control);
