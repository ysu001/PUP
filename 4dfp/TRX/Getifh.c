...
/****************************************************************************************/
/*$Header: /data/petsun4/data1/src_solaris/TRX/RCS/Getifh.c,v 1.22 2011/04/09 00:16:43 avi Exp $*/
/*$Log: Getifh.c,v $
 * Revision 1.22  2011/04/09  00:16:43  avi
 * filename buffer length increased from MAXL to 2*MAXL
 *
...
static char	rcsid[] = "$Id: Getifh.c,v 1.22 2011/04/09 00:16:43 avi Exp $";
void Getifh_rcs (void) {printf ("%s\n", rcsid);}

int Getifh (char *imgfile, IFH *ifhdr) {
	FILE	*fp;
	char	*str, ifhfile[2*MAXL], line[2*MAXL], parameter[MAXL];
	int 	i;
	int	mmppix_flag = 0, center_flag = 0, endian_flag = 0;
	int	debug = 0;

	getroot (imgfile, ifhfile);
	strncat (ifhfile, ".4dfp.ifh", 2*MAXL - 1);
	...
	memset (line, '\0', 2*MAXL);
	while (fgets (line, 2*MAXL, fp)) {
		if ((str = strrchr (line, '\n'))) *str ='\0';
...
int writeifhmc (char *program, char *outfile, int *imgdim, float *voxdim, int orient, float *mmppix, float *center) {
	FILE		*ifhfp;
	char		*ptr, imgroot[2*MAXL];

	getroot (outfile, imgroot);
	strncat (imgroot, ".4dfp.ifh", 2*MAXL - 1);
