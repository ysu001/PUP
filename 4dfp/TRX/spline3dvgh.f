...
c Copyright 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010,
c 2011
...
c Revision 1.12  2011/07/07  01:09:01  avi
c restore malloc/free
c
c Revision 1.11  2008/02/03  00:48:27  avi
c turn off ldebug in splinezf()
c
...
c Initial revision
c
c $Header: /data/petsun4/data1/src_solaris/TRX/RCS/spline3dvgh.f,v 1.12 2011/07/07 01:09:01 avi Exp $
...
c     end
      ...
      write (*,"('$Id: spline3dvgh.f,v 1.12 2011/07/07 01:09:01 avi Exp $')")
      return
      end
...
c     real*4 a(0:511),b(0:511),f(0:256)
      real*4 a(0:nx-1),b(0:nx-1),f(0:nx)
      pointer (pa,a),(pb,b),(pf,f)

      pa=malloc(4*nx)
      pb=malloc(4*nx)
      pf=malloc(4*(nx+1))
c     if (nx.gt.512) stop 'splinex: nx may not exceed 512'
      if (pa.eq.0.or.pb.eq.0.or.pf.eq.0) stop 'splinex: memory allocation error'
...
c     real*4 a(0:511),b(0:511),f(0:256)
      real*4 a(0:ny-1),b(0:ny-1),f(0:ny)
      pointer (pa,a),(pb,b),(pf,f)

      pa=malloc(4*ny)
      pb=malloc(4*ny)
      pf=malloc(4*(ny+1))
c     if (ny.gt.512) stop 'spliney: ny may not exceed 512'
      if (pa.eq.0.or.pb.eq.0.or.pf.eq.0) stop 'spliney: memory allocation error'
...
c     real*4 a(0:511),b(0:511),f(0:256)
      real*4 a(0:2*nz),b(0:2*nz),f(0:2*nz)
      pointer (pa,a),(pb,b),(pf,f)
      ...
      pb=malloc(4*nzp)
      pf=malloc(4*(nzp+1))
c     if (nzp.gt.512) stop 'splinezf: nzp may not exceed 512'
      if (pa.eq.0.or.pb.eq.0.or.pf.eq.0) stop 'splinezf: memory allocation error'
   ...
   31 continue
      ...
      real*4 imag(0:nx-1,0:ny-1,0:nz-1),d2zi(0:nx-1,0:ny-1,0:nz-1)
c     real*4 a(0:262143),u(0:511)
      real*4 a(0:nz-1,0:nz-1),u(0:nz-1)
      pointer (pa,a),(pu,u)
      pa=malloc(4*nz*nz)
      pu=malloc(4*nz)
...
c         d2zi(ix,iy,iz)=d2zi(ix,iy,iz)+a(iz+nz*jz)*u(jz)
        enddo
      enddo
   11 continue
