c$Header: /data/petsun4/data1/src_solaris/imglin/RCS/t4_sub.f,v 1.10 2008/05/13 06:05:20 avi Exp $
c$Log: t4_sub.f,v $
c Revision 1.10  2008/05/13  06:05:20  avi
c linux gcc compliant ('\0' -> char(0))
c
c Revision 1.9  2007/08/09  06:34:10  avi
c change float read formats "(f)" -> "(f32.4)" to achieve gcc v4 compliance
c
...
c Initial revision
      ...
      write (*,"('$Id: t4_sub.f,v 1.10 2008/05/13 06:05:20 avi Exp $')")
      return
      end
...
c     l=index(t4_file,char(0))-1
c     write(*,"('l=',i4)")l
c     if(l.le.0.or.l.eq.256)l=lnblnk(t4_file)
c     write(*,"('l=',i4)")l
c     write(*,"('t4_file=',a60)")t4_file(1:l)
    ...
    1 read(8,"(a)",end=9)string
      ...
      k=lnblnk(string)
      read(string(7:k),"(f32.4)",err=9)param(13)
    9 close(8)
      end
    ...
    1 read(8,"(a)",end=9)string
      if(string(1:6).ne.'scale:')goto 1
      k=lnblnk(string)
      read(string(7:k),"(f32.4)",err=9)scale
      iget_t4_scale=0
    9 close(8)
      end
