/*$Header: /data/petsun4/data1/src_solaris/nifti_4dfp/RCS/nifti1.h,v 1.2 2010/06/18 21:00:59 coalsont Exp $*/
/*$Log: nifti1.h,v $
 * Revision 1.2  2010/06/18  21:00:59  coalsont
 * added define checks to avoid __attribute__ on non-gcc compilers
 *
 * Revision 1.1  2009/07/08  00:56:33  coalsont
 * Initial revision
 **/
/* Tim Coalson [tsc5yc@mst.edu], NRG, 29 June 2009: modified to use custom stdint for systems without it */
/* Kevin P. Barry [ta0kira@users.berlios.de], BMCLAB, 22 May 2009

  NOTE: i modified the data structures because the "official" header doesn't
  take into account integer sizes or alignment. this introduces a dependence on
  gcc, but who cares. */
...
#ifndef NOT_GCC
} __attribute__ ((packed));                   /**** 348 bytes total ****/
#else
};
#endif
 ...
 */
#ifndef NOT_GCC
struct nifti1_extender { uint8_t extension[4] ; } __attribute__ ((packed));
#else
struct nifti1_extender { uint8_t extension[4] ; };
#endif
 ...
 */
struct nifti1_extension {
   int32_t  esize ; /*!< size of extension, in bytes (must be multiple of 16) */
   int32_t  ecode ; /*!< extension code, one of the NIFTI_ECODE_ values       */
